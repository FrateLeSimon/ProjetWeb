...
class EntrepriseController {
    ...
    public function getEntreprises($start_from, $records_per_page, $search_query = null) {
        if ($search_query) {
            ...
            $sql = "SELECT entreprise.id_entreprise, nom_entreprise, secteur_activite, ville, code_postal, logo, description FROM Entreprise LEFT JOIN est_localisé_à ON entreprise.id_entreprise = est_localisé_à.id_entreprise LEFT JOIN adresse ON est_localisé_à.id_adresse = adresse.id_adresse ORDER BY entreprise.id_entreprise ASC LIMIT $start_from, $records_per_page";
            $stmt = $this->conn->prepare($sql);
        }
        $stmt->execute();
